...
* $T_{c2w}\in\mathbb{R}^{4\times 4}$：相机外参，表示 camera-to-world 变换
* $H,W$：图像高宽
* $I\in\mathbb{R}^{H\times W\times 3}$：输出图像，每个像素是 RGB 向量
...
* $\mathbf{x}\in\mathbb{R}^3$：场景中某个采样点的 3D 位置
* $\mathbf{d}\in\mathbb{R}^3$：这个采样点的被观察方向
* $\mathbf{c}\in\mathbb{R}^3$：这个采样点的颜色
* $\sigma\in\mathbb{R}$：这个采样点的体密度
...
* 外参 $T_{c2w}=[R\ |\ \mathbf{t}] \in \mathbb{R}^{4\times 4}$
* 其中 $R\in\mathbb{R}^{3\times 3},\ \mathbf{t}\in\mathbb{R}^3$
...
* $\mathbf{O}\in\mathbb{R}^{N_r\times 1\times 3}$
* $\mathbf{D}\in\mathbb{R}^{N_r\times 1\times 3}$
* $\mathbf{t}\in\mathbb{R}^{N_r\times N_c\times 1}$
* $\mathbf{X}\in\mathbb{R}^{N_r\times N_c\times 3}$
...
$$
\gamma(\mathbf{d})\in\mathbb{R}^{3(2L_d+1)}
$$
...
所以维度通常为：
* $\gamma(\mathbf{x})$: $3(2\times 10+1)=63$
* $\gamma(\mathbf{d})$: $3(2\times 4+1)=27$

### 输出

* 编码后采样点位置：$(N_r,N_c,C_x=63)$
* 编码后采样点被观测方向：$(N_r,N_c,C_d=27)$

---

## Step 4: coarse MLP 预测辐射场
...
* 编码后位置：$(N_r,N_c,C_x)$，通常 $C_x=63$
* 编码后方向：$(N_r,N_c,C_d)$，通常 $C_d=27$
...
* $\mathbf{f}\in\mathbb{R}^{C_f}$：中间特征
* $\sigma\in\mathbb{R}$：密度
* $\mathbf{c}\in\mathbb{R}^3$：RGB

这里可以这样理解:

* $\sigma$ 回答的是: 这个位置有没有东西, 挡不挡光
* $\mathbf{c}$ 回答的是: 如果看到这里, 它应该贡献什么颜色
...
## Step 5: coarse 体渲染并得到权重

这一步不只是为了出一张粗图，更重要的是为后面的 fine sampling 提供 PDF 权重。

也就是说, coarse 网络虽然叫 "粗网络", 但它不是可有可无的;  
它一方面先给出一个粗糙的渲染结果, 另一方面还负责告诉系统: 射线上哪些区域更值得继续细采样。

### 输入

* `rgb_coarse`: $(N_r,N_c,3)$
* `sigma_coarse`: $(N_r,N_c)$
* `z_vals_coarse`: $(N_r,N_c)$
...
* `weights_coarse`: $(N_r,N_c)$
* `rgb_map_coarse`: $(N_r,3)$
* `depth_map_coarse`: $(N_r,)$

这里的 `weights_coarse` 很关键;  
它表示每条射线上的哪些采样点真正对最终像素贡献更大, 后面的 fine sampling 就是根据它来做的。

---

## Step 6: 层次采样 / fine sampling
...
* `rgb_fine_samples`: $(N_r,N,3)$
* `sigma_fine_samples`: $(N_r,N,1)$ 或 $(N_r,N)$

---

## Step 8: fine 体渲染得到最终像素颜色
...
$$
\hat{\mathbf{C}}(\mathbf{r})=\sum_i w_i\mathbf{c}_i + \Big(1-\sum_i w_i\Big)\mathbf{c}_{bg}
$$

其中白背景时 $\mathbf{c}_{bg}=(1,1,1)$。
